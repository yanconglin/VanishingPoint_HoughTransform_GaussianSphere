...
def init_ht_convs(layers, laplacian_init=False):
    if not laplacian_init:
        ...
        # print('HT_CONV Laplacian intialization')
...
class HTCONVBlock(nn.Module):
    def __init__(self, in_planes, planes):
        super().__init__()
        # why no batchnorm: the same as HT-LCNN paper
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=5, stride=1, padding=2, dilation=1, groups=1, bias=True)
        # self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=5, stride=1, padding=2, dilation=1, groups=1, bias=True)
...
class HT_CONV(nn.Module):
    def __init__(self, inplanes, outplanes):
        ...
        # # # Q: why not 9x1 as in HT-LCNN / why 3x3 or 5x5?
        # # # In theory: one should use convs over the offset dimension only (pls refer to the HT-LCNN (radon/inver-radon transform for details).
        # # # Assumption: the HT quantization there is more fined-grained (180 angles) than the HT-LCNN (60 angles); make sense to use 2D kernels?
        # # # Answer: 9x1/3x3/5x5 - not much difference. No idea why I ended up 5x5, guessed I forgot to undo, sorry... :(
