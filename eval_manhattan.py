...
def main():
    ...
    # assert C.io.dataset.upper() in ["WIREFRAME", "SCANNET", "YUD"]
    assert C.io.dataset.upper() in ["WIREFRAME", "SCANNET"]
    ...
    if args["--output"]: np.savez(os.path.join(args["--output"]), err=errors)
    y = (1 + np.arange(len(errors))) / len(loader) / M.batch_size / C.io.num_vpts

    if not args["--noimshow"]:
        plt.plot(errors, y, label="HT_GaussianSphere")
        print(" | ".join([f"{AA(errors, y, th):.3f}" for th in [1, 3, 5, 10]]))
        plt.xlim(0.0, 10.0, 1.0)
